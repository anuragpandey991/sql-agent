...
def run_out_of_scope():
    ...
    ), "Expected out_of_scope classification"
...
def run_write_operation():
    ...
    print("Intent:", state["intent"])
    assert (
        state["intent"] == "write_operation"
    ), "Expected write_operation classification"
...
def run_hallucination_trap():
    print(
        "=== TEST 4: Schema-violating question (discount doesn't exist) ==="
    )

    state = create_initial_state(
        "What discount did each customer receive?"
    )
    ...
    print("Validation errors:", state["validation_errors"])
    print(
        "Final answer (if NO_QUERY triggered):",
...
def run_multiturn_context():
    ...
    state2 = create_initial_state(
        "Now only show ones from Germany",
